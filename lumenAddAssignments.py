from reclaim_sdk.models.task import ReclaimTask
from reclaim_sdk.client import ReclaimClient
from datetime import datetime

token = "eyJhbGciOiJIUzI1NiJ9..xHcP5HSmLpc3LWApAh5NNo4SOYLGdvBaW2L-s8R4lLk"  # noqa

ReclaimClient(token=token)


assignments = []

file1 = open("data.txt", "r")
Lines = file1.readlines()

counter = 0

assignments = []

currentAssignment = {}

for line in Lines:
    # reset assignment object
    if counter % 8 == 0:
        currentAssignment = {}

    # title
    if counter % 8 == 1:
        start = line.find("HW")
        if start < 0:
            start = line.find("Exam")
        end = line.find("<", start)
        title = line[start:end]

        if not title:
            counter += 7
            continue

        currentAssignment["title"] = "(MATH 125) " + title

    # grade
    if counter % 8 == 2:
        start = line.find(">") + 1
        grade = line[start: line.find("<", start)]

        currentAssignment["grade"] = grade

    if counter % 8 == 5:
        start = line.find(">") + 1
        dueDate = line[start: line.find("<", start)]
        currentAssignment["dueDate"] = dueDate

    if counter % 8 == 7:
        if (
            currentAssignment["grade"] == "0%"
            and datetime.strptime(currentAssignment["dueDate"], "%m/%d/%y %H:%M%p")
            > datetime.now()
        ):
            # It is also possible to create an object without using a context manager.
            task = ReclaimTask()

            task.name = currentAssignment["title"]

            if currentAssignment["title"].find("HW") > -1:
                task.duration = 0.75
            else:
                task.duration = 5

            task.min_work_duration = 0.5
            task.max_work_duration = 1.5
            task.is_work_task = False
            task.start_date = datetime.now()
            task.due_date = datetime.strptime(
                currentAssignment["dueDate"], "%m/%d/%y %H:%M%p"
            )

            # updating tasks
            # tasks = ReclaimTask.search(title=currentAssignment["title"])

            # for task in tasks:
            #     print("current task name", task)
            #     print("current work status", task.is_work_task)
            #     task.name = "\U00002795 " + task.name
            #     task.is_work_task = False
            #     print("Updated task name", task)
            #     print("Updated work status", task.is_work_task)

            # Then the object needs to be saved manually to the API.
            task.save()

            assignments.append(currentAssignment)
    counter += 1

# print(assignments)
